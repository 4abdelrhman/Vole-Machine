...
void Machine::operations(int opcode, int reg, int xy) {
    ...
    switch (opcode) {
        ...
        case 2: // LOAD R, immediate XY
            R.Set_Value(reg, xy);
            break;
        case 3: // STORE R, M[XY] (or output to screen if XY == 0x01)
        ...
        case 0xB: // COMP R, XY: Compare R3 with R0 and decide to print or jump
            if (R.Get_Value(reg) == R.Get_Value(0)) {
                cout << "Result: " << R.Get_Value(1) << endl; // Print the result from R1
                exit(0); // End program
            } else {
                // Decrement R3 and jump back to the addition step
                R.Set_Value(reg, R.Get_Value(reg) - 1);  // Decrement the counter in R3
                JUMP(reg, xy);  // Jump back to the addition step
            }
            break;
        case 0xC: // HALT
            cout << "Program halted." << endl;
